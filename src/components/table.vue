    ...
    <div class="table-responsive">
      <table class="table table-striped">
        <thead>
          <tr>
            <th>
              <input type="checkbox" @change="ToggleFlag()" :checked="arr.length == 10" />
            </th>
            <th>
              <select class="form-select form-select-sm" @change="SelectChange($event)">
        ...
        <tbody>
          <tr v-for="item in data.slice(props.first, props.end)" :key="item.id">
            <td colspan="2">
              <input
                type="checkbox"
                :checked="arr.includes(item.id)"
                :value="item.id"
                @change="ManageArr(item.id)"
              />
            </td>
...
const ToggleFlag = () => {
  ...
  if (Flag.value) {
    for (let i = props.first; i < props.end; i++) {
